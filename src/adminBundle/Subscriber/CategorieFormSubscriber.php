...
class CategorieFormSubscriber implements EventSubscriberInterface
{
    ...
    public function postSetData(FormEvent $event){
        $entity=$event->getData();// recupere les donner du formulaire
        $form= $event->getForm();// recupere le formulaire

         // ici on test si le id existe dans on est dans la mehode edit
        //  apres il faut supprimer le champ position
        if($entity->getId()){
            $form->remove('position');// on supprimer le champ position lors que update (edit)
            $form->add('description');// on ajoute le champ description dans la formulaire
        }else{// cas c'est ma creation c-a-d getid!=null
